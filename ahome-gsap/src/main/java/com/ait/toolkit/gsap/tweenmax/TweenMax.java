...
/**
 * 
 * TweenMax extends TweenMax, adding many useful (but non-essential) features like timeScale(), repeat(), repeatDelay(), yoyo(), updateTo(), and more. It also activates many extra
 * plugins by default, making it extremely full-featured. Any of the plugins can work with TweenMax too, but TweenMax saves you the step of activating the common ones like
 * CSSPlugin, RoundPropsPlugin, and BezierPlugin. Since TweenMax extends TweenMax, it can do ANYTHING TweenMax can do plus more. The syntax is identical. You can mix and match
 * TweenMax and TweenMax in your project as you please, but if file size is a concern it is best to stick with TweenMax unless you need a particular TweenMax-only feature.
 * 
 */
public class TweenMax extends Animation {

	private static TweenMax INSTANCE;
	private static final TweenMaxResources resources = GWT.create(TweenMaxResources.class);
	...
	/**
	 * Method for creating a TweenMax instance that animates to the specified destination values (from the current values).
	 * 
	 * @param target
	...
	/**
	 * Method for creating a TweenMax instance that tweens backwards - you define the BEGINNING values and the current values are used as the destination values which is great for
	...
	/**
	 * Method for creating a TweenMax instance that allows you to define both the starting and ending values (as opposed to to() and from() tweens which are based on the target's
	...
	private native TweenMax _fromTo(JavaScriptObject target, double duration, TweenMaxConfig fromConfig, TweenMaxConfig toConfig)/*-{
		var peer = this.@com.ait.toolkit.core.client.JsObject::getJsObj()();
		var obj = peer.fromTo(target, duration,
				fromConfig.@com.ait.toolkit.core.client.JsObject::getJsObj()(),
				toConfig.@com.ait.toolkit.core.client.JsObject::getJsObj()());
	...
	private static native boolean isLoaded()/*-{
		if (typeof $wnd.TweenMax === "undefined" || $wnd.TweenMax === null) {
			return false;
		}
		return true;
	}-*/;
